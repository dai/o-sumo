...
describe('getBashoStatus', () => {
  it('treats dates before the published schedule as upcoming', () => {
    expect(getBashoStatus(torikumiArchive, new Date('2026-08-31T00:00:00.000Z')).kind).toBe('upcoming');
  });

  it('treats an in-basho JST date as live and exposes the day number', () => {
    const status = getBashoStatus(torikumiArchive, new Date('2026-09-16T03:00:00.000Z'));
    expect(status.kind).toBe('live');
    expect(status.day).toBe(4);
  });

  it('treats dates after senshuraku as final', () => {
    expect(getBashoStatus(torikumiArchive, new Date('2026-10-01T00:00:00.000Z')).kind).toBe('final');
  });
});
