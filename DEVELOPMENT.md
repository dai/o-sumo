...
- `app/components/GreetingSection.tsx`: MonomosuSection 内にネストされた「編集者より」最新記事カード (`blogFeed.items[0]` を h3 で強調表示)
- `app/components/MonomosuSection.tsx`: ホームの「物申す」独立セクション (visible h2、座布団リアクション + textarea drawer、GreetingSection を内包)
...
- 9月場所開催中（2026-09-13〜2026-09-27）は `daily-data-update.yml`（JST 13/15/17/19 時）と `realtime-torikumi-direct-update.yml`（JST 15:00–18:57 の3分間隔 = 試行頻度、runner queue で coalesce され実効発火は 10-15 回/日）が cron で起動する。`workflow_dispatch` は fallback として残置。9月27日最終結果の翌日確認後、cron 停止と `workflow_dispatch` のみの運用復帰は別 PR で行う。
