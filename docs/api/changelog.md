...
- `ai-input=yes` で検索・回答生成時の参照を許可。`ai-train=no` は維持
- 公開スキルの月キー・対応月・JSTの日付解決を修正し、ビルド時のdigest更新を確認
- 月別・日別のMarkdownを既存場所データから生成。日付・掲載状況・予定/結果別の更新時刻・取組を掲載
- 過去場所の番付で現行場所のデータを流用しないよう修正し、HTMLフォールバックのMarkdown誤配信を防止
- README・運用ガイドに対応範囲とMCP/A2Aの非提供機能を明記。JSON APIのスキーマ変更なし
