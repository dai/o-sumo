...
現在の更新フロー（2026-07-01 JST まで自動実行停止）:

- 日次更新（取組予定のみ）: `daily-data-update.yml` を手動実行（`workflow_dispatch`）
- 高頻度更新（取組結果 + 番付）: `realtime-torikumi-update.yml` を手動実行（`workflow_dispatch`）
