...
- 日次更新: `.github/workflows/daily-data-update.yml`
  ...
  - 番付と取組予定データを更新
  - 更新 PR を作成後、自動マージ（auto-merge）を有効化
- 高頻度更新: `.github/workflows/realtime-torikumi-update.yml`
  - 15:00-20:00 JST に 30 分間隔
  - 取組結果だけを更新
- どちらも `main` へ直接 push せず、更新用ブランチから PR を作成
- 更新 PR で必須チェックが通過したあとに自動マージされる（ブランチ保護ルール準拠）
...
- `docs/api/v1.md`: API 仕様
- `skills/osumo-api/SKILL.md`: API 利用 Skill
