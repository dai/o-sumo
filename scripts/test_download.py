#!/usr/bin/env python3
"""
Download rikishi profile images - test with first 5 rikishi.
"""
...
def fetch_profile_page(rikishi_id: int) -> str | None:
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                print(f"[ERROR] Failed to fetch profile {rikishi_id}: {exc}")
                return None
    ...
    for attempt in range(3):
        try:
            with urlopen(req, timeout=30) as res:
                ...
                print(f"[OK] Downloaded: {output_path.name} ({len(data)} bytes, type: {content_type})")
                return True
        except Exception as exc:
            if attempt == 2:
...
def process_rikishi(rikishi: dict) -> bool:
    ...
    print(f"\n[INFO] Processing {name} ({rikishi_id}, {rank})...")
...
def main():
    """Main execution"""
    # Test with first 5 rikishi
    limit = 5
    print(f"Testing with first {limit} rikishi...")
    print(f"Loading rikishi list from {RIKISHI_JSON}...")
    rikishi_list = load_rikishi_list(limit=limit)
    print(f"Found {len(rikishi_list)} rikishi to process")
