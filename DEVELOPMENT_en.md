...
- `app/components/GreetingSection.tsx`: "From the Editor" callout nested as h3 inside MonomosuSection (featured card from `blogFeed.items[0]`)
- `app/components/MonomosuSection.tsx`: standalone "Have Your Say" section on homepage (visible h2, zabuton reaction + textarea drawer, hosts the GreetingSection)
