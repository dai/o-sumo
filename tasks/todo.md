...
- GREEN: strengthened `python scripts/preflight_current_basho_test.py` は20 tests OK。公式BashoInfoを両部門から取得し、場所名・年/月・初日・千秋楽を公式年間日程と照合する。route/sitemapは実在するcurrent source pathと仮想target pathの衝突を検査し、公式取得失敗・不一致時はlocal fallbackせず `BLOCKED` / exit 1になる構成にした。
